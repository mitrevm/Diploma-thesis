...
# ---------------------- Helper Functions ----------------------
def find_related_rows(df, n=15, random_seed=30):
    results = []
    ...
    for _, row in pair_rows.iterrows():
        ...
        ).sample(n, random_state=random_seed)#.sample(n, random_state=42)#.head(n)
...
def print_nan_columns(name, X, feature_names):
    mask = np.isnan(X)
    ...
    if mask.any():
        ...
        nan_counts = mask.sum(axis=0)

        bad_cols = [(feature_names[i], nan_counts[i]) for i in range(len(feature_names)) if nan_counts[i] > 0]

        print("\n📊 Columns with NaNs:")
        for col, cnt in bad_cols:
            print(f"{col}: {cnt}")

        # also show worst column
        worst = max(bad_cols, key=lambda x: x[1])
        print("\n🔥 Worst column:", worst)
...
def create_data_set(selected_df: pd.DataFrame,
    ...
    def process_df(input_df, is_training=False, model=""):
        ...
        for _, row in input_df.iterrows():
            ...
            if not os.path.exists(capri_ss_path):
                 capri_ss_path = (
                f"/d/hpc/home/mm5129/Diplomska/runs/collected_caprieval/"
                f"07_caprieval_run_new_model_0_{row['trgb1_gen']}_{row['trgc1_gen']}/capri_ss.tsv"
            )
            ...
            try:
                ...
                df_row = agg.T.stack(future_stack=True).to_frame().T
                
                #df_row = df_full[feature_cols].copy()
                ...
                # Load embeddings
                trgb1_emb = load_embedding(row['trgb1_gen'], model).ravel()
                trgc1_emb = load_embedding(row['trgc1_gen'] if "nc" not in row['trgc1_gen']
                                           else f"TRGC_{row['trgc1_gen']}", model).ravel()
                                           
                
                if np.isnan(trgb1_emb).any():
                    print(f"🚨 NaNs in trgb1_emb: {row['trgb1_gen']}")
                    print("count:", np.isnan(trgb1_emb).sum())

                if np.isnan(trgc1_emb).any():
                    print(f"🚨 NaNs in trgc1_emb: {row['trgc1_gen']}")
                    print("count:", np.isnan(trgc1_emb).sum())
               ...
               # df_row["pae"] = (row["pae_0_1"] + row["pae_1_0"])/2
                ...
                emb_vector = trgb1_emb.tolist() + trgc1_emb.tolist()
                #emb_vector = trgb1_emb - trgc1_emb
                ...
                expected_len = len(embeded_cols)
                actual_len = len(trgb1_emb.tolist() + trgc1_emb.tolist())

                if actual_len != expected_len:
                    print("🚨 EMBEDDING LENGTH MISMATCH")
                    print("expected:", expected_len, "got:", actual_len)
        ...
        if all_data:
            ...
            return main_df, embeded_cols, trgb1_emb_len, trgc1_emb_len
        else:
            # Return empty DataFrame with proper columns
            columns = [f"{col}_{stat}" for col in required_columns
                       for stat in ['mean', 'min', 'max', 'std']] + ['compatible'] + embeded_cols
            return pd.DataFrame(columns=columns), embeded_cols, trgb1_emb_len, trgc1_emb_len

    # Process both selected and remaining
    selected_all_data, embeded_cols, trgb1_emb_len, trgc1_emb_len = process_df(selected_df, is_training=False, model=model)
    remaining_all_data, _, trgb1_emb_len, trgc1_emb_len = process_df(remaining_df, is_training=True, model=model)

    return selected_all_data, remaining_all_data, embeded_cols, trgb1_emb_len, trgc1_emb_len



def train_logreg_and_predict(selected_df: pd.DataFrame,
                             remaining_df: pd.DataFrame,
                             embeded_cols: list, c: float, pca:int, trgb1_emb_len: int, trgc1_emb_len: int):
   ...
   # ['pae_0_1', 'pae_1_0']
    ...
    X_train_emb = remaining_df[embeded_cols].to_numpy()#+ [f"trgc1_emb_{i}" for i in range(trgc1_emb_len)]].values          # embeddings
  ...
  # ['pae_0_1', 'pae_1_0']
   ...
   # ].values
    ...
    X_selected_emb = selected_df[embeded_cols].to_numpy()#+ [f"trgc1_emb_{i}" for i in range(trgc1_emb_len)]].values
   ...
   #    
    ...
    # X_selected_emb,
    # extr_selected])
    pca_r = TruncatedSVD(n_components=8, random_state=42) 
    X_train_final = remaining_df[embeded_cols].to_numpy()
    X_selected_final = selected_df[embeded_cols].to_numpy()
    X_train_emb_pca = pca_r.fit_transform(X_train_final)
    X_selected_emb_pca = pca_r.transform(X_selected_final)
    X_train_final = X_train_emb_pca
    X_selected_final = X_selected_emb_pca

   # X_train_final = X_train_num_scaled
    #X_selected_final = X_selected_num_scaled
    if c is not None:
    ...
    train_feature_names = (
    #list(X_selected_num) +   # see note below
    list([f"trgb1_emb_{i}" for i in range(trgb1_emb_len)]) 
)

   ## print_nan_columns("X_train_final", X_train_final, train_feature_names)
    
    if np.isnan(X_train_final).any():
        print("\n🚨 NaNs in X_train_final 🚨")
        print("Shape:", X_train_final.shape)

        nan_rows = np.isnan(X_train_final).any(axis=1)
        print("Rows with NaNs:", np.where(nan_rows)[0])

        print("Sample bad row:")
        print(X_train_final[nan_rows][0])

        raise RuntimeError("NaNs in train data")
        
    clf.fit(X_train_final, y_train)
    if np.isnan(X_selected_final).any():
        print("\n🚨 NaNs in X_selected_final 🚨")
        print("Shape:", X_selected_final.shape)

        nan_rows = np.isnan(X_selected_final).any(axis=1)
        print("Rows with NaNs:", np.where(nan_rows)[0])

        print("Sample bad row:")
        print(X_selected_final[nan_rows][0])

        raise RuntimeError("NaNs in test data")
...
def run_pipeline(original, i, percent, n_in_training, n_in_test, df, c, model, pca):
    ...
    en, dva, embeded_cols, trgb1_emb_len, trgc1_emb_len = create_data_set(selected_df, remaining_df,
                                            n_in_training=n_in_training,
                                            n_in_test=n_in_test, model=model)
    if en.empty or dva.empty:
        return None

    avg_prob_1, avg_prob_0 = train_logreg_and_predict(en, dva, embeded_cols, c, pca, trgb1_emb_len, trgc1_emb_len)
...
def parse_args():
    ...
    parser.add_argument("--random_seed", type=int, default=30, help="Random seed")
...
if __name__ == "__main__":
    ...
    r = find_related_rows(df, random_seed=random_seed)
    ...
    for n_in_training in [n_in_training]:
        for n_in_test in [n_in_test]:
            ...
            folder = f"/d/hpc/home/mm5129/A_pairs_fixed/embeddings_show/pca_8/csv"
