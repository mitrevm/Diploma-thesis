...
def train_logreg_and_predict(selected_df: pd.DataFrame,
    ...
    X_train_num = remaining_df[['euclidean', 'dot_prod']].values
    X_train_emb = remaining_df["cos_sim"].values.reshape(-1, 1)  # embeddings
    y_train = remaining_df[target_col].values

    X_selected_num = selected_df[['euclidean', 'dot_prod']].values
    ...
    for n_in_training in [n_in_training]:
        for n_in_test in [n_in_test]:
            ...
            if "percent" not in results_df.columns:
                ...
                continue
            folder = f"/d/hpc/home/mm5129/A_pairs_fixed/OnlyEmbeddings/model_{model}/test_embeddings_data/csv"
