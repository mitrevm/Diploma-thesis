...
def find_related_rows(df, n=15):
    ...
    for _, row in pair_rows.iterrows():
        ...
        ).sample(n, random_state=42)#.head(n)
...
def create_data_set(selected_df: pd.DataFrame,
    ...
    def process_df(input_df, is_training=False, model=""):
        ...
        embeded_cols = [f"trgb1_emb_{i}" for i in range(trgb1_emb_len)] + \
                       [f"trgc1_emb_{i}" for i in range(trgc1_emb_len)]
        ...
        for _, row in input_df.iterrows():
            ...
            try:
                ...
                df_selected = df[
                    required_columns + ['compatible', 'cos_sim', 'euclidean', 'dot_prod']
                ].copy()
        ...
        if all_data:
            return pd.concat(all_data, ignore_index=True), embeded_cols
        else:
            return pd.DataFrame(columns=required_columns + ['compatible'] +
                                list(embeded_cols)), list(embeded_cols)


    selected_all_data, _ = process_df(selected_df, is_training=False, model=model)
    remaining_all_data, _ = process_df(remaining_df, is_training=True, model=model)

    return selected_all_data, remaining_all_data, _


def train_and_predict(selected_df: pd.DataFrame,
                      remaining_df: pd.DataFrame,
    ...
    X_train_num = remaining_df[feature_cols + ['euclidean', 'dot_prod']].values
    X_train_emb = remaining_df["cos_sim"].values.reshape(-1, 1)  # embeddings
    y_train = remaining_df[target_col].values

    X_selected_num = selected_df[feature_cols + ['euclidean', 'dot_prod']].values
    ...
    if clf_name == "svm":
        # Use probability=True so output stays compatible with current pipeline.
        if c is not None:
...
# ---------------------- Parallel Wrapper ----------------------
def run_pipeline(original, i, percent, n_in_training, n_in_test, df, c, model, clf_name):
    global global_var
    global_var += 1
    ...
    avg_prob_1, avg_prob_0 = train_and_predict(en, dva, embeded_cols, c, clf_name)
...
def parse_args():
    ...
    parser.add_argument("--clf", type=str, default="logistic", choices=["logistic", "svm"],
        ...
        for n_in_test in [n_in_test]:
            ...
            results = Parallel(n_jobs=10, verbose=10)(
                delayed(run_pipeline)(original, i, percent,
                                      n_in_training, n_in_test, df, c, model, clf_name)
                for (original, i, percent) in tasks
            )

            results = [res for res in results if res is not None]
            if not results:
                print("No successful pipeline results for this train/test setting; skipping export.")
                continue
            results_df = pd.DataFrame(results)
            if "percent" not in results_df.columns:
                print("Missing 'percent' in results; skipping export.")
                continue
            folder = f"/d/hpc/home/mm5129/A_pairs_fixed/Hybrid/model_{model}/test_embeddings_data/csv"
