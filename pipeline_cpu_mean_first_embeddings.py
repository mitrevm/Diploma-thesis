...
def train_logreg_and_predict(selected_df: pd.DataFrame,
    ...
    X_train_num = remaining_df[
      # ['euclidean', 'dot_prod'] + 
        ['desolv_mean', 'vdw_mean', 'elec_mean', 
    'improper_mean', 'rdcs_mean', 'rg_mean', 'sym_mean', 'total_mean', 'vdw_min', 'vdw_max', 'vdw_std', 
   ...
   # +['pae']
    ...
    X_train_emb = remaining_df["cos_sim"].values.reshape(-1, 1) #remaining_df[embeded_cols].values          # embeddings
    y_train = remaining_df[target_col].values

    X_selected_num = selected_df[
       #['euclidean', 'dot_prod'] + 
        ['desolv_mean', 'vdw_mean', 'elec_mean', 'improper_mean', 'rdcs_mean', 'rg_mean', 
    'sym_mean', 'total_mean', 'vdw_min', 'vdw_max', 'vdw_std', 'elec_min', 'elec_max', 'elec_std', 'improper_min', 'improper_max', 'improper_std', 
    'rdcs_min', 'rdcs_max', 'rdcs_std', 'rg_min', 'rg_max', 'rg_std', 'sym_min', 'sym_max', 'sym_std', 'total_min', 'total_max', 'total_std']
    +['pae_0_1', 'pae_1_0']
   # +['pae']
    ].values
    X_selected_emb = selected_df["cos_sim"].values.reshape(-1, 1) #selected_df[embeded_cols].values
...
if __name__ == "__main__":
    ...
    for n_in_training in [n_in_training]:
        for n_in_test in [n_in_test]:
            ...
            folder = f"/d/hpc/home/mm5129/A_pairs_fixed/Hybrid/mean/model_{model}/no_embeddings&new_features&alphafold&iptm_for_each_c={c}&random_state=30/csv"
