    ...
    def process_df(input_df, is_training=False, model= ""):
        ...
        for _, row in input_df.iterrows():
            ...
            try:
                ...
                #df = df[feature_cols].mean().to_frame().T
...
def train_logreg_and_predict(selected_df: pd.DataFrame,
                             remaining_df: pd.DataFrame,
                             embeded_cols: list, c: float, pca_components: int ):
    ...
    # PCA
   # print(f"klk={len(embeded_cols)}")
    pca = TruncatedSVD(n_components=pca_components, random_state=42)    
    X_train_emb_pca = pca.fit_transform(X_train_emb)
    X_selected_emb_pca = pca.transform(X_selected_emb)
...
# ---------------------- Parallel Wrapper ----------------------
def run_pipeline(original, i, percent, n_in_training, n_in_test, df, c, model, pca_components):
    global global_var
    global_var += 1
    ...
    avg_prob_1, avg_prob_0 = train_logreg_and_predict(en, dva, embeded_cols, c, pca_components)
...
def parse_args():
    ...
    parser.add_argument("--pca", type=int, default=0.1, help="Regularization parameter")
        ...
        for n_in_test in [n_in_test]:
            ...
            results = Parallel(n_jobs=10, verbose=10)(
                delayed(run_pipeline)(original, i, percent,
                                      n_in_training, n_in_test, df, c, model, pca_components)
                for (original, i, percent) in tasks
            )

            results = [res for res in results if res is not None]
            results_df = pd.DataFrame(results)
            folder = f"/d/hpc/home/mm5129/model_{model}/csv_embeddings_pca_on_embeddings_{model}_pca={pca_components}"
