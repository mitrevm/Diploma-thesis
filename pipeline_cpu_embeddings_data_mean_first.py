    ...
    def process_df(input_df, is_training=False, model= ""):
        ...
        for _, row in input_df.iterrows():
            ...
            try:
                ...
                df = df_raw[required_columns].mean().to_frame().T
                df['compatible'] = row['pair']

    
                emb_trb1 = load_embedding(row['trgb1_gen'], model)
                   
                if "nc" in row["trgc1_gen"]:
                    print(f"Loading TRGC_{row['trgc1_gen']}")
                    emb_trgc1 = load_embedding(f"TRGC_{row['trgc1_gen']}", model)
                else:
                    emb_trgc1 = load_embedding(row['trgc1_gen'], model)

                
                emb_trb1_flat = emb_trb1.ravel()
                emb_trgc1_flat = emb_trgc1.ravel()
    ...
    for n_in_training in [n_in_training]:
        for n_in_test in [n_in_test]:
            ...
            if "percent" not in results_df.columns:
                ...
                continue
            folder = f"/d/hpc/home/mm5129/A_pairs_fixed/Hybrid/model_{model}/test_embeddings_data_with_mean_first/csv"
